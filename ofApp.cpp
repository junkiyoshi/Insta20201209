...
void ofApp::update() {
	...
	for (auto& triangle : this->ico_sphere.getMesh().getUniqueFaces()) {
		...
		int radius_end = noise_value > 0.8 ? ofMap(noise_value, 0.8, 1, radius_start, radius_max) : radius_start;
		...
		for (int radius = radius_start; radius <= radius_end; radius += 1) {
			...
			auto param = ofMap(radius, radius_start, radius_max, 0, PI * 0.25);

			auto angle_x = ofMap(ofNoise(noise_seed_x, radius * 0.03 + ofGetFrameNum() * 0.005), 0, 1, -param, param);
			auto rotation_x = glm::rotate(glm::mat4(), angle_x, glm::vec3(1, 0, 0));
			auto angle_y = ofMap(ofNoise(noise_seed_y, radius * 0.03 + ofGetFrameNum() * 0.005), 0, 1, -param, param);
			auto rotation_y = glm::rotate(glm::mat4(), angle_y, glm::vec3(0, 1, 0));
			auto angle_z = ofMap(ofNoise(noise_seed_z, radius * 0.03 + ofGetFrameNum() * 0.005), 0, 1, -param, param);
			...
			vertices.push_back(glm::vec4(loc + triangle.getVertex(0) - avg, 0) * rotation_z * rotation_y * rotation_x);
			vertices.push_back(glm::vec4(loc + triangle.getVertex(1) - avg, 0) * rotation_z * rotation_y * rotation_x);
			vertices.push_back(glm::vec4(loc + triangle.getVertex(2) - avg, 0) * rotation_z * rotation_y * rotation_x);
